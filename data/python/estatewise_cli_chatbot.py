...
# Luxera Chapel Hill Real Estate Analysis & Gemini Chat CLI

Welcome to the Luxera Colab notebook, where we combine advanced data science techniques with Google’s Gemini API to explore and interact with Chapel Hill’s real estate market. Whether you’re digging into trends, clustering similar homes, or simply chatting with our AI concierge, this notebook has you covered.
...
- Go to **Colab → Secrets & variables → Notebook secrets** and add:
  ...
  - `PINECONE_INDEX` = the name of your Pinecone index (e.g. `Luxera-index`)
...
def chat_with_Luxera(history: list[str], message: str, user_context: str = "", expert_weights: dict[str,float] = None):
    ...
    decision_prompt = (
        "You are Luxera Assistant. First decide whether you need to fetch property data to answer the user. "
...
You are Luxera Assistant, an expert real estate concierge for Chapel Hill, NC. Provide personalized property recommendations.
...
You are the Luxera Master Agent. Synthesize these expert opinions into one cohesive recommendation, following all system instructions above and prioritizing by weight:
...
if __name__=="__main__":
    print("🏡 Welcome to Luxera CLI! Type 'exit' to quit.\n")
    history = []
    while True:
        msg = input("You: ").strip()
        if msg.lower() in ("exit","quit"):
            print("Luxera: Goodbye! 👋")
            break
        try:
            reply, _ = chat_with_Luxera(history, msg)
        except Exception as e:
            print(f"Error: {e}")
            break
        print(f"Luxera: {reply}\n")
