...
- `frontend/lib/api.ts`
- direct frontend fetch callers under `frontend/pages/`
...
- MCP contract change:
  ...
  - `cd agentic-ai && npm run build` when consumed there

- A2A/HTTP contract change:
  - `cd agentic-ai && npm run build`
  - validate a realistic HTTP or CLI flow if possible
  - validate MCP `a2a.*` bridge when relevant
