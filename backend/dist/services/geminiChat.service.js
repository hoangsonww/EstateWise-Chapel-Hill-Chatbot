...
async function chatWithLuxera(history, message, userContext = {}, expertWeights = {}) {
    ...
    const dataNotFetched = (0, lib_1.default)(message);
    let propertyContext;
    let rawResults;
    if (!dataNotFetched || !userContext.rawResults) {
        [propertyContext, rawResults] = await Promise.all([
            (0, queryProperties_1.queryPropertiesAsString)(message, 30),
            (0, queryProperties_1.queryProperties)(message, 30),
        ]);
    ...
    if (rawResults.length > 0) {
        const featureVectors = rawResults.map((r) => {
            const m = r.metadata;
            ...
            const livingArea = typeof m.livingArea === "string"
        ...
        featureVectors.forEach((vec) => vec.forEach((val, i) => {
            if (val < mins[i])
                mins[i] = val;
            if (val > maxs[i])
                maxs[i] = val;
        }));
        const normalized = featureVectors.map((vec) => vec.map((val, i) => maxs[i] === mins[i] ? 0 : (val - mins[i]) / (maxs[i] - mins[i])));
        // Cluster
  ...
  `;
    ...
    ${expertResults
        .map((r) => `**${r.name}** (weight: ${weights[r.name].toFixed(2)}):
    ${r.text}`)
        .join("\n\n")}
  ...
  `;
    ...
    const resultOrTimeout = await Promise.race([
        mergePromise,
        new Promise((resolve) => setTimeout(() => resolve({ timeout: true }), Math.max(0, remaining))),
    ]);
    let finalText;
    if (resultOrTimeout.timeout) {
        // timed out → pick highest‐weight expert
        const best = expertResults.reduce((a, b) => weights[b.name] > weights[a.name] ? b : a);
        finalText = `${best.text}`;
    }
    else {
        finalText = resultOrTimeout.response.text();
    }
