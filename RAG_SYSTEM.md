...
- [Live Data Freshness Augmentation](#live-data-freshness-augmentation)
...
📡 **Live Zillow Snapshot Layer** - Local fresh-listing artifact (`data/live-zillow/output`) exposed via MCP `live.zillow.search`
...
graph TB
    subgraph "Data Sources"
        ...
        L[Live Zillow Snapshot<br/>data/live-zillow/output]
    end
    ...
    subgraph "RAG Pipeline"
        ...
        X[Live Snapshot Retrieval<br/>MCP live.zillow.search]
    end
...
## Live Data Freshness Augmentation

For freshness-sensitive listing prompts, EstateWise can enrich retrieval with a local Zillow live snapshot artifact produced by `data/live-zillow/fetch_live_zillow.mjs`.

```mermaid
sequenceDiagram
    participant Scheduler as Cron/CI
    participant Fetch as fetch_live_zillow.mjs
    participant FS as live_zillow_snapshot.normalized.json
    participant MCP as MCP live.zillow.search
    participant Agent as agentic-ai runtime

    Scheduler->>Fetch: run pipeline
    Fetch->>FS: write fresh snapshot
    Agent->>MCP: live.zillow.search(q/city/state)
    MCP->>FS: read snapshot
    FS-->>MCP: normalized listings + generatedAt
    MCP-->>Agent: fresh candidates + provenance
```

- This path is additive to vector + graph retrieval.
- Snapshot freshness is visible via `generatedAt`.
- Backend chat path remains bounded by existing timeout controls and does not execute live scraping inline.
...
💡 _These 3 properties are in Cluster 1 (mid-range family homes)
in the same neighborhood, offering consistent community vibes._
```
...
- ✅ Neo4j (knowledge graph) - _optional, system works without it_
...
- ✅ Users need to understand _why_ recommendations are made
...
- 💡 Understand _why_ homes are suggested (transparency)
